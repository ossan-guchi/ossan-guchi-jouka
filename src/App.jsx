...
export default function OssanGuchiJoukaApp() {
  ...
  const result = useMemo(() => {
    ...
    const empathyMap = {
      hardworking: "今日もよく耐えました。言いたいことを飲み込んで、ちゃんと大人をやってきたんですね。",
      boss: "理不尽な一言は、意外と心に残ります。あなたが弱いのではなく、ちゃんと責任感があるからこそ傷ついたのだと思います。",
      family: "家でも職場でも気を張っていると、逃げ場がなくなりますよね。ここでは、かっこつけなくて大丈夫です。"
    };
    ...
    const advice = "次にやることは1つだけで十分です。今日は『反論する』『解決する』よりも、風呂に入る、寝る、散歩するなど、自分を回復させる行動を優先してください。";

    const cleanse = "この愚痴は、お焚き上げします。明日まで持ち越さなくていい。今日のあなたは、今日を生き抜いただけで十分です。";
      ...
      <AnimatePresence>
        {burning && (
          ...
          >
            ...
            >
              <p className="mb-3 text-sm text-orange-200">お焚き上げ中</p>
            ...
            <div className="absolute bottom-0 h-72 w-full overflow-hidden">
              {[...Array(26)].map((_, i) => (
                <motion.div
                  ...
                  animate={{ y: -40 - (i % 5) * 20, opacity: [0, 1, 0], scale: [0.8, 1.4, 0.6] }}
                  transition={{ duration: 1.6 + (i % 4) * 0.2, repeat: 1, delay: i * 0.035 }}
                  className="absolute bottom-0 rounded-full bg-gradient-to-t from-red-600 via-orange-400 to-yellow-200 blur-[1px]"
                  style={{ left: `${8 + (i * 3.7) % 86}%`, width: `${28 + (i % 4) * 14}px`, height: `${90 + (i % 5) * 35}px` }}
                />
              ))}
            </div>
            ...
            {[...Array(18)].map((_, i) => (
              <motion.span
                ...
                animate={{ opacity: [0, 1, 0], y: -260, x: (i % 2 === 0 ? 1 : -1) * (80 + i * 3) }}
            ...
            >
              ...
              <p className="text-2xl font-bold">空へ還しました</p>
              <p className="mt-2 text-sm text-stone-300">明日まで持ち越さなくて大丈夫です</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <main className="relative z-10 mx-auto max-w-4xl">
        ...
        >
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-orange-300/20 bg-stone-900/70 px-4 py-2 shadow-sm">
            ...
            <span className="text-sm font-medium text-orange-100">おっさんの愚痴浄化所</span>
          </div>
          <h1 className="mb-3 text-3xl font-bold tracking-tight md:text-5xl">
            愚痴は、溜めずに燃やせ。
          </h1>
          <p className="text-base text-stone-300 md:text-lg">
            仕事・上司・家族・人生のモヤモヤを吐き出して、固定メッセージで受け止め、最後はお焚き上げで空へ還します。
          </p>
        </motion.div>
          ...
          <CardContent className="space-y-5 p-5 md:p-7">
            <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
              <button
                ...
                className={`rounded-xl px-4 py-3 text-sm font-medium transition ${mode === "hardworking" ? "bg-orange-500/25 ring-2 ring-orange-300/40" : "bg-stone-800 hover:bg-stone-700"}`}
              >
                仕事疲れモード
              </button>
              <button
                onClick={() => setMode("boss")}
                className={`rounded-xl px-4 py-3 text-sm font-medium transition ${mode === "boss" ? "bg-orange-500/25 ring-2 ring-orange-300/40" : "bg-stone-800 hover:bg-stone-700"}`}
              >
                上司・職場モード
              </button>
              <button
                onClick={() => setMode("family")}
                className={`rounded-xl px-4 py-3 text-sm font-medium transition ${mode === "family" ? "bg-orange-500/25 ring-2 ring-orange-300/40" : "bg-stone-800 hover:bg-stone-700"}`}
              >
                家庭・人生モード
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <Button onClick={submitGuchi} className="rounded-xl bg-stone-100 px-5 py-6 text-base text-stone-950 hover:bg-white">
                <Send className="mr-2 h-4 w-4" />
                愚痴を入力
              </Button>
              <Button onClick={reset} variant="outline" className="rounded-xl border-stone-600 bg-transparent px-5 py-6 text-base text-stone-100 hover:bg-stone-800">
        ...
        <AnimatePresence>
          {submitted && savedGuchi && !burning && (
            ...
            >
              <ResponseCard icon={<Flame className="h-5 w-5" />} title="受け止めます" text={result.empathy} />
              <ResponseCard icon={<Wind className="h-5 w-5" />} title="愚痴の整理" text={result.summary} />
              <ResponseCard icon={<MessageCircle className="h-5 w-5" />} title="おっさん向け実用アドバイス" text={result.advice} />
              <ResponseCard icon={<Sparkles className="h-5 w-5" />} title="お焚き上げ前の言葉" text={result.cleanse} highlight />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="mx-auto max-w-4xl">
          ...
          >
            ...
            <span className="relative mt-1 block text-xs font-medium text-orange-100 md:text-sm">
              {savedGuchi ? "押すと愚痴が炎で燃えて空へ消えます" : "先に『愚痴を入力』を押してください"}
            </span>
          </motion.button>
        </div>
      </div>
...
function ResponseCard({ icon, title, text, highlight = false }) {
  return (
    <Card className={`rounded-2xl border-orange-200/10 shadow-lg ${highlight ? "bg-orange-950/70" : "bg-stone-900/80"}`}>
      <CardContent className="p-5">
        <div className="flex items-start gap-3">
          <div className={`rounded-2xl p-2 ${highlight ? "bg-orange-500/20 text-orange-200" : "bg-stone-800 text-orange-300"}`}>{icon}</div>
          <div>
...
function SecurityCard({ icon, title, text }) {
  return (
    <Card className="rounded-2xl border-emerald-300/10 bg-stone-900/70 shadow-md">
      <CardContent className="p-5">
        <div className="flex items-start gap-3">
          <div className="rounded-2xl bg-emerald-500/10 p-2 text-emerald-300">{icon}</div>
          <div>
